...
{
  //useState를 2개 만든다 (이미지들, 현재슬라이드번호)
  ...
  async function fetchImages(getUrl) {
    ...
    let response = await fetch(`${getUrl}?page=${props.page}&limit=${props.limit}`); //await 이거 완료될 때까지 기다려라
      ...
      <BsArrowLeftCircleFill className = 'arrow arrow-left' onClick = {goPrev}/>
      {
        images && images.length? (
          images.map((image, idx)=>{
            return(
              <img key = {image.id} src = {image.download_url} className = {curSlide === idx? "current-image" : "current-image hide-current-image"}/>
            )
          })
        )
      : <div>이미지 로딩중 ...</div>
